...
> Source: **Azure Host — Azure Host Node** dashboard, chapter **Direct Drive Performance Tables** (4 queries across 4 sub-groups).

Each KQL block is preserved verbatim from the dashboard. Substitute params (`{globalFrom}`, `{nodeId}`, etc.) with case values, then execute via vm-kusto-query / kusto_runner.py / replay.py.

---

## Agent Start Operation Performance DD (P50, 90, 99)

### Agent Start Operations Details Direct Drive (P50, 90, 99)

_Widget purpose:_ Agent Start Operation Performance DD (P50, 90, 99)

Cluster: `azcore.centralus.kusto.windows.net` · Database: `Fa` · Type: `Table`
Source panel: `Direct Drive Performance Tables > Agent Start Operation Performance DD (P50, 90, 99)`

```kusto
let directDriveClusters = cluster('azcore.centralus.kusto.windows.net').database('SharedWorkspace').StorageTenantCatalogSnapshot()
| where Type == "StorageDirectFlash"
| distinct Tenant, ClusterName;
let directDriveTenantName = directDriveClusters | project Tenant;
let directDriveClusterName = directDriveClusters | project ClusterName;
...
| where TenantName in (directDriveTenantName)
| where StartDate < FinishDate
...
| where Cluster in (directDriveClusterName)
...
**Signal filters seen in KQL:** `Type == "StorageDirectFlash"` · `Scenario == "Reboot"`

---

## Container Workflow Details DD (P50, 90, 99)

### Container Workflow Details Direct Drive (P50, 90, 99)

_Widget purpose:_ Container Workflow Details DD (P50, 90, 99)

Cluster: `azurecm.kusto.windows.net` · Database: `AzureCM` · Type: `Table`
Source panel: `Direct Drive Performance Tables > Container Workflow Details DD (P50, 90, 99)`
...
| where PreciseTimeStamp between (StartDate .. FinishDate)
| where Tenant in (directDriveClusterName)
...
**Signal filters seen in KQL:** `Type == "StorageDirectFlash"`

---

## IfxOperationV2 Performance DD (P50, 90, 99)

### IfxOperationV2 Performance Details Direct Drive (P50, 90, 99)

_Widget purpose:_ IfxOperationV2 Performance DD (P50, 90, 99)

Cluster: `azcore.centralus.kusto.windows.net` · Database: `Fa` · Type: `Table`
Source panel: `Direct Drive Performance Tables > IfxOperationV2 Performance DD (P50, 90, 99)`
...
**Signal filters seen in KQL:** `Type == "StorageDirectFlash"` · `OperationName == "AgentpCreateContainerWorker"`

---

## Node Workflow Details DD (P50, 90, 99)

### NodeWorkflow Details Direct Drive (P50, 90, 99)

_Widget purpose:_ Node Workflow Details DD (P50, 90, 99)

Cluster: `azurecm.kusto.windows.net` · Database: `AzureCM` · Type: `Table`
Source panel: `Direct Drive Performance Tables > Node Workflow Details DD (P50, 90, 99)`
