...
> Source: **Azure Host - Azure VM** dashboard, chapter **(top-level)** (1 queries across 1 sub-groups).
...
**Tables:** `MycroftContainerSnapshot`, `GuestOSDetailEtwTable`, `LogContainerSnapshot`, `LogContainerPolicySnapshot`, `LogClusterSnapshot`
**Aggregations:** `summarize arg_max(PreciseTimeStamp, creationTime, roleInstanceName, subscriptionId, contai by containerId` · `summarize arg_min(PreciseTimeStamp, memoryInMB, hostMemoryReservationInMBytes) by Tenant, policyInstanceName`
**Output columns:** `Tenant1`, `Tenant2`, `policyInstanceName`, `ContainerId`
