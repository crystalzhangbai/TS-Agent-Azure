# Azure Networking Queries — NRP, VPN, ExR, AppGW, vWAN, SLB, CDN, DDoS
...
|----------------------------|------------|------------------------|
| 通过私有IP查找VM、find VM by private IP、VM IP lookup、NIC IP mapping | `Nrp.mdsnrp` | WriteOperationResponseEtwEvent (NIC write responses contain assigned private IP) |
...
1. For most network CRUD problems: first query `Nrp.mdsnrp.QosEtwEvent` (closest to the ASC Operations view), then drill into `FrontendOperationEtwEvent` via `correlationId`, and finally trace back to the `Armprodgbl/ARM` layer.
2. VPN/ExR/AppGW problems are all in `Hybridnetworking.aznwmds`, located via `GatewayId` or `ServiceKey`.
3. Fairfax (Government) equivalent clusters: `Aznwff.kusto.usgovcloudapi.net` (database: `aznwmds`) covers all NRP, VPN, ExR, AppGW tables; `Armff.kusto.usgovcloudapi.net` (database: `armff`) covers ARM.
...
```kusto
// Note: macro-expand must be run directly on armprodgbl.eastus.kusto.windows.net / database: ARMProd
// It cannot use cluster('...').database('...') prefix — connect to the cluster first.
...
| `WriteOperationResponseEtwEvent` | NIC write operation responses; contains full response body including assigned private IP, NIC-to-VM mapping | TIMESTAMP, SubscriptionId, Uri, Request, Response, HttpStatusCode, ResourceType, ResourceName, ResourceGroup |
...
```kusto
// Pivot from CRP via ClientOperationId
// (when the failing operation comes from compute, e.g.
//  Failed-to-Delete-VM_Start-Stop, FabricCallback-InternalPowerOff,
//  NetworkingInternalOperationError) — ClientOperationId == CRP/Compute operationId
cluster("nrp").database("mdsnrp").FrontendOperationEtwEvent
| where PreciseTimeStamp between(datetime({Start}) .. datetime({End}))
| where ClientOperationId == "{ClientOperationId}"
| project PreciseTimeStamp, Message, TraceSource, SourceVersion
| order by PreciseTimeStamp asc
```

**Fairfax**: `https://Aznwff.kusto.usgovcloudapi.net` → database: `aznwmds` (QosEtwEvent + FrontendOperationEtwEvent)

### Key KQL: Find VM Resource ID by Private IP

> **Note:** `LogContainerSnapshot` (AzureCM) does **not** store private IPs.
> Private IPs are managed by NRP and recorded in `WriteOperationResponseEtwEvent`
> when a NIC is created/updated. Retention is ~185 days.
> NIC naming convention: `{vmName}-{nicIndex}` (e.g. suffix `-0` = primary NIC, `-1` = secondary).

```kusto
// Find VM Resource ID by Private IP
// Usage: Replace _subscriptionId and _privateIP, then run
let _subscriptionId = "{SubscriptionId}";
let _privateIP = "{PrivateIP}";
...
| where * has _privateIP
| summarize arg_max(PreciseTimeStamp, Uri) by SubscriptionId
| parse Uri with * "/subscriptions/" SubId "/resourcegroups/" ResourceGroup "/providers/Microsoft.Network/networkInterfaces/" NicName "?" *
| extend VMName = extract(@"^(.+)-\d+$", 1, NicName)
| extend VMResourceId = strcat("/subscriptions/", SubId, "/resourceGroups/", ResourceGroup, "/providers/Microsoft.Compute/virtualMachines/", VMName)
| project PreciseTimeStamp, PrivateIP=_privateIP, NicName, VMName, VMResourceId, ResourceGroup
```
...
## Netperf → NetPerfKustoDB (Host Pingmesh)

**URI**: `https://netperf.kusto.windows.net` → database: `NetPerfKustoDB`  
**Access**: same as the Azure VM platform Kusto bundle.  
**Purpose**: Pingmesh — server-to-server probe latencies across the Azure backbone. Used to confirm whether elevated host networking latency is impacting VM disk / cross-host comms.

| Table | Purpose | Key Columns |
|-------|---------|-------------|
| `AzPingMeshServerStatus` | Host pingmesh probe results (avg RTT in microseconds) | timestamp, serverName, serverIP, avgRTTInMicroseconds, avgPayloadRTTInMicroseconds, avgSyncRTTInMicroseconds |

### AzPingMeshServerStatus — Host pingmesh latency anomalies (>10 ms RTT)

Look up the host's IP/hostname via `Azuredcm.AzureDCMDb.ResourceSnapshotV1` (NodeId → IP), then join into `AzPingMeshServerStatus` to find probes where the host saw avg RTT ≥ 10,000 µs (normal host pingmesh is 300–1500 µs).
...
| where serverName == tolower(lower_hostname)
| project-rename FoundTimestamp = timestamp
| project FoundTimestamp, NodeId=ResourceId, Hostname=serverName, serverIP,
    avgPayloadRTTInMicroseconds, avgRTTInMicroseconds, avgSyncRTTInMicroseconds
| take 5
```

Interpretation:
- Normal host RTT: 300–1500 µs. Threshold of 10,000 µs (10 ms) used by the [Host Node Investigation_Perf TSG](https://supportability.visualstudio.com/AzureIaaSVM/_wiki/wikis/AzureIaaSVM/495968) to confirm elevated host networking latency.
- If hits land in the customer's impact window, expect elevated `CurAvgTxLatInms` in Host Analyzer (VM → storage path traverses host NIC) and elevated XStore client/server latency.
- 5–20x baseline RTT typically maps to a 5x amplification of VM disk transmit latency.
- Alternative UI: NetVMA (select host node IP → "Pingmesh" button on the right).
...
| `Netperf.kusto.windows.net` | `NetPerfKustoDB` | shared with VM platform bundle | **Host Pingmesh** (`AzPingMeshServerStatus`) — see dedicated section above |
